...
class LocalComponentDependencyMetadataTest extends Specification {
    ...
    def "returns this when same target requested"() {
        ...
        def dep = new LocalComponentDependencyMetadata(componentId, selector, "from", null, "to", [] as Set, [], false, false, true, false)
    ...
    def "selects the target configuration from target component"() {
        def dep = new LocalComponentDependencyMetadata(componentId, Stub(ComponentSelector), "from", null, "to", [] as Set, [], false, false, true, false)
    ...
    def "selects the target configuration from target component which matches the attributes"() {
        def dep = new LocalComponentDependencyMetadata(componentId, Stub(ComponentSelector), "from", null, null, [] as Set, [], false, false, true, false)
    ...
    def "revalidates default configuration if it has attributes"() {
        def dep = new LocalComponentDependencyMetadata(componentId, Stub(ComponentSelector), "from", null, Dependency.DEFAULT_CONFIGURATION, [] as Set, [], false, false, true, false)
...
Configuration 'default': Required key 'other' and found incompatible value 'nothing'.""")
    ...
    def "revalidates explicit configuration selection if it has attributes"() {
        def dep = new LocalComponentDependencyMetadata(componentId, Stub(ComponentSelector), "from", null, 'bar', [] as Set, [], false, false, true, false)
        def defaultConfig = defaultConfiguration()
        def toFooConfig = Stub(LocalConfigurationMetadata) {
            getName() >> 'foo'
            getAttributes() >> attributes(key: 'something')
...
Configuration 'bar': Required key 'something' and found incompatible value 'something else'.""")
    ...
    def "fails to select target configuration when not present in the target component"() {
        ...
        def dep = new LocalComponentDependencyMetadata(fromId, Stub(ComponentSelector), "from", null, "to", [] as Set, [], false, false, true, false)
        def toComponent = Stub(ComponentResolveMetadata)
    ...
    def "applies exclude rules when traversing the from configuration"() {
        ...
        def moduleExclusions = new ModuleExclusions(new DefaultImmutableModuleIdentifierFactory())
        def dep = new LocalComponentDependencyMetadata(componentId, Stub(ComponentSelector), "from", null, "to", [] as Set, [exclude1, exclude2], false, false, true, false)
